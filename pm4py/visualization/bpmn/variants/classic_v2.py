...
def add_bpmn_node(graph, n, font_size, include_name_in_events, endpoints_shape):
    ...
    if isinstance(n, BPMN.Task):
        ...
        graph.node(n_id, label=node_label, shape=endpoints_shape, style="filled", fillcolor="green", fontsize=font_size)
    elif isinstance(n, BPMN.EndEvent):
        graph.node(n_id, label=node_label, shape=endpoints_shape, style="filled", fillcolor="orange", fontsize=font_size)
    elif isinstance(n, BPMN.Event):
        graph.node(n_id, label=node_label, shape="ellipse", style="filled", fillcolor='grey', fontsize=font_size)
        ...
        # graph.node(n_id, label="X", xlabel=node_label, shape="diamond", fontsize=font_size)
        label_str = "X"
        if node_label:
            label_str = f"\n\n{label_str}\n\n{node_label}"
        graph.node(n_id, label=label_str, shape="diamond", fontsize=font_size, fixedsize="true")
...
def apply(bpmn_graph: BPMN, parameters: Optional[Dict[Any, Any]] = None) -> graphviz.Digraph:
    ...
    endpoints_shape = exec_utils.get_param_value(Parameters.ENDPOINTS_SHAPE, parameters, "doubleoctagon")
    ...
    for e in edges:
        ...
        if n_id_1 in added_nodes and n_id_2 in added_nodes:
            ...
            for flow in bpmn_graph.get_flows():
                if flow.source == e[0] and flow.target == e[1]:
                    label = flow.get_name()
                    break
            
            viz.edge(n_id_1, n_id_2, label=label, fontsize=font_size)
